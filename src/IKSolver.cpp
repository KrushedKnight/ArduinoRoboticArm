...
bool IKSolver::cosrule(double opposite, double adjacent1, double adjacent2,
                       double &angle) {
...
JointAngles IKSolver::analyticalSolve(double x, double y, double z,
                                      double phi) {
  ...
  if (!cosrule(Constants::ELBOW_LENGTH, R, Constants::SHOULDER_LENGTH,
               elbowInteriorAngle)) {
  ...
  if (!cosrule(R, Constants::SHOULDER_LENGTH, Constants::ELBOW_LENGTH,
               wristInteriorAngle)) {
    std::cerr << "cos rule error!";
    std::exit(1);
  }
  ...
  double verify_x =
      cos(baseAngle) *
      (Constants::SHOULDER_LENGTH * cos(shoulderAngle) +
  ...
  double verify_y =
      ...
      (Constants::SHOULDER_LENGTH * cos(shoulderAngle) +
       ...
       Constants::WRIST_LENGTH * cos(shoulderAngle + elbowAngle + wristAngle));
  double verify_z =
      Constants::BASE_HEIGHT + Constants::SHOULDER_LENGTH * sin(shoulderAngle) +
  ...
  std::cout << "Wrist reference point -> P_x: " << P_x << ", P_y: " << P_y
            << ", P_phi: " << P_phi * 180.0 / M_PI << "°" << std::endl;
  std::cout << "Wrist position (xw, yw): (" << xw << ", " << yw
            << "), Polar R: " << R
            << ", alpha: " << shoulderInteriorAngle * 180.0 / M_PI << "°"
            << std::endl;

  std::cout << "Shoulder inner angle (beta): "
            << elbowInteriorAngle * 180.0 / M_PI << "°" << std::endl;
  std::cout << "Elbow inner angle (gamma): "
            << wristInteriorAngle * 180.0 / M_PI << "°" << std::endl;

  std::cout << "DOWN solution -> Shoulder: " << shoulderAngleDown * 180.0 / M_PI
            << "°, Elbow: " << elbowAngleDown * 180.0 / M_PI
            << "°, Wrist: " << wristAngleDown * 180.0 / M_PI << "°"
            << std::endl;
  ...
  std::cout << "FK check: (" << verify_x << ", " << verify_y << ", " << verify_z
            << ")" << std::endl;
  std::cout << "Position error: "
            << sqrt(pow(x - verify_x, 2) + pow(y - verify_y, 2) +
                    pow(z + Constants::BASE_HEIGHT - verify_z, 2))
            << std::endl;

  JointAngles result{baseAngle * Constants::RADIANS_TO_DEGREES,
